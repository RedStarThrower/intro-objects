...
var getWordsArray = function(textString) {
    var wordsArray = textString.split(" ")
    return wordsArray
}

var getElCounts = function(arr) {
    var countObj = {}
...
var getCounts = function(textString) {
    var lowerCase = textString.toLowerCase()
    var wordsArray = getWordsArray(lowerCase)
    return getElCounts(wordsArray)
}
...
var macbeth = {title: "Macbeth", author: "Shakespeare", year: 1620}

var meetsCriteria = function(inputObj, criteriaObj) {
    for (var property in criteriaObj) {
        if (criteriaObj[property] !== inputObj[property]) {
            return false
        }
    }
    return true
...
// var result = meetsCriteria(macbeth, {title: "Macbeth", year: 1620})
// console.log(result) => true

var where = function(objectsArray, criteriaObj) {
    var outputArray = []
    for (var i = 0; i < objectsArray.length; i++) {
        var singleObject = objectsArray[i]
        if(meetsCriteria(singleObject, criteriaObj)) {
            outputArray.push(singleObject)
        }
    }
    return outputArray
}
